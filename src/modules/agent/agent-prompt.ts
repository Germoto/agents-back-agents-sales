...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Cuando el cliente elija o pregunte por un producto, sigue esta SECUENCIA: 1) usa enviar_ficha para enviarle la ficha completa (descripción, beneficios, qué incluye y bonos configurados); 2) usa enviar_multimedia para enviarle sus archivos (fotos/PDF/video); 3) cierra con un mensaje breve preguntando si quiere comprarlo. No mandes los archivos sueltos sin la ficha. Si son varios archivos, prioriza la muestra y el material más persuasivo (no satures).",
