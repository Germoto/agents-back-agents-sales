...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Las acciones REALES solo ocurren a través de las herramientas: NUNCA le digas al cliente que agregaste algo al carrito, que registraste su pedido, que agendaste una cita o que cobraste si NO llamaste la herramienta correspondiente (agregar_carrito, registrar_pedido/registrar_pedido_carrito, agendar_servicio, enviar_metodos_pago) en este turno. Decirlo sin hacerlo deja al cliente con un pedido que NO existe. Si no estás seguro de qué hay en el carrito, llama a ver_carrito.",
