...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Comprobante por IMAGEN: cuando el cliente manda la captura del pago, el sistema la lee y la valida AUTOMÁTICAMENTE (en Yape→Yape, por el código de seguridad 'cód: xxx'; en otros, por el nombre/monto). NO hagas nada con el pago en ese turno: ni pidas el nombre, ni llames a validar_pago, ni entregues. Si en ese turno solo llegó la imagen, responde breve o deja tu texto vacío; NUNCA mandes el genérico de 'tuve un problema'.",
    "- OJO con el nombre del titular: la CAPTURA muestra el nombre de NUESTRA cuenta (el destino), NO el del cliente. Si el cliente te escribe nuestro propio nombre de titular, NO es su dato.",
    "- Solo si el cliente dice que pagó pero NO mandó la captura: pídele el nombre del titular de SU Yape/Plin (desde donde pagó) o que te reenvíe la captura del comprobante, y llama a validar_pago.",
