  ...
  switch (vertical) {
    ...
    case "STREAMER":
      return "Rubro STREAMER/SUSCRIPCIONES: el catálogo son PLANES (agrupados por plataforma/servicio en la categoría). Compara planes por periodo (mensual/anual), tier, nº de pantallas y calidad; recomienda el más conveniente. " +
        "PLANES/MODALIDADES: si un producto lista 'modalidades' (ej. periodos como mensual/trimestral/anual, o tipos como 1 perfil / 2 perfiles / cuenta completa) con distinto precio, para mostrarlos usa enviar_ficha — YA envía la lista 'Planes y precios'. NO reescribas esa lista de planes/precios en tu propio texto (evita duplicarla): tras enviar la ficha, responde solo con UNA frase breve para que el cliente elija. No digas que el precio es 'fijo' ni inventes. Cuando el cliente elija, al agregar al carrito pasa la modalidad en `modifiers` como {group:'Plan', option:'<la que eligió>'}; el precio de la línea se ajusta solo. Si no lista modalidades, se vende al precio base. " +
