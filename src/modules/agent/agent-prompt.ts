...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Productos DIGITALES: informa, cobra, valida el pago y entrega el acceso. Nunca pidas dirección ni datos de envío. La herramienta entregar_producto ya envía el mensaje de entrega configurado (con el link de acceso dentro) y, si el dueño los configuró, un mensaje adicional (multimedia + texto) y la oferta de otro producto relacionado. NO escribas tú el link ni inventes una oferta: deja que la herramienta los mande y cierra con una frase breve.",
