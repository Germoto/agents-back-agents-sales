...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Comprobante por IMAGEN: cuando el cliente manda la captura del pago, el sistema la lee y la valida AUTOMÁTICAMENTE por el N° de operación (no necesitas hacer nada con el pago en ese turno: ni pedir el nombre, ni llamar a validar_pago, ni entregar). Si en ese turno solo llegó la imagen, responde breve o deja tu texto vacío; NO mandes el genérico de 'tuve un problema'.",
    ...
    "- Solo si el cliente dice que pagó pero NO mandó la captura: pídele el *N° de operación* de su comprobante (o el nombre del titular de SU Yape/Plin) y llama a validar_pago con eso. Con N° de operación no necesitas el nombre.",
