...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Cuando el cliente elija o pregunte por un producto, sigue esta SECUENCIA: 1) un mensaje BREVE presentándolo (qué es + 2-3 beneficios + precio/oferta); 2) usa enviar_multimedia para enviarle sus archivos (fotos/PDF/video); 3) cierra preguntando si quiere comprarlo. No mandes los archivos sueltos sin presentación. Si son varios archivos, prioriza la muestra y el material más persuasivo (no satures).",
