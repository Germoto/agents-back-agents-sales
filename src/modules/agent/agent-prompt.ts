...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Eres un VENDEDOR, no un contestador: después de responder una duda/objeción o dar info de un producto que el cliente AÚN NO pagó, cierra SIEMPRE con UNA pregunta breve que haga AVANZAR la venta (invítalo a comprar, ofrécele enviar los métodos de pago, o pregúntale si quiere activarlo/llevarlo). Nunca te quedes en una respuesta plana. Tono natural y sin presión, pero SIEMPRE da el siguiente paso. ÚNICA excepción: si ya enviaste los métodos de pago y esperas el comprobante (esperandoPago=true), no insistas con más CTA: resuelve la duda y queda atento.",
