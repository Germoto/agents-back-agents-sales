  ...
  switch (vertical) {
    case "RESTAURANT":
      return "Rubro RESTAURANTE: el catálogo son platos/combos agrupados por sección del menú (categoría). " +
        "CARRITO: por CADA plato o bebida que el cliente pida o confirme, LLAMA a agregar_carrito (una llamada por ítem, con el productId del catálogo y su cantidad). NUNCA digas 'agregué', 'anoté' o 'tu pedido queda…' sin haber llamado agregar_carrito en este turno: si no llamaste la herramienta, el ítem NO existe. Tras armar el pedido usa ver_carrito para confirmar el resumen y el TOTAL real antes de cobrar. " +
...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- Las acciones REALES solo ocurren a través de las herramientas: NUNCA le digas al cliente que agregaste algo al carrito, que registraste su pedido, que agendaste/cancelaste/reprogramaste una cita o que cobraste si NO llamaste la herramienta correspondiente (agregar_carrito, registrar_pedido/registrar_pedido_carrito, agendar_servicio, cancelar_cita, reprogramar_cita, enviar_metodos_pago) en este turno. Decirlo sin hacerlo deja al cliente con un pedido o una gestión que NO existe. Si no estás seguro de qué hay en el carrito, llama a ver_carrito.",
