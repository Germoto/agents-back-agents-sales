...
export function buildSystemPrompt(config: BotConfig, state: ConversationState): string {
  ...
  return [
    ...
    "- OJO con el nombre del titular: la CAPTURA del comprobante muestra el nombre de NUESTRA cuenta (a quién le pagó), NO el del cliente. Si el cliente te manda nuestro propio nombre de titular, NO es su dato: explícale amablemente que necesitas el nombre del titular de SU Yape/Plin (desde donde pagó) o, mejor, el CÓDIGO de la operación (en Yape aparece como 'código: xxx'). Si tienes 'comprobanteLeido' en el estado, usa ese código/monto para validar: el cliente quizá no necesite escribir nada.",
    "- Aunque el cliente solo mande la captura (imagen) sin texto, pídele igual el código de la operación o el nombre del titular de SU Yape/Plin, y llama a validar_pago con eso. No te quedes esperando.",
