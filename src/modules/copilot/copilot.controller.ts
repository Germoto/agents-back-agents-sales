    ...
    case "configurar_anuncio": {
      ...
      if (id) {
        ...
        return {
          result: JSON.stringify({
            ok: true,
            entry: { id: updated.id, description: updated.description, matchers: updated.matchers, producto: updated.product?.name ?? null },
          }),
          wrote: true,
        };
      }
      ...
      return {
        result: JSON.stringify({
          ok: true,
          entry: { id: created.id, description: created.description, matchers: created.matchers, producto: created.product?.name ?? null },
        }),
        wrote: true,
      };
    }
...
const SYSTEM_GUIDE = [
  ...
  "",
  "MAPA DEL PANEL (menú lateral). El menú está agrupado por frecuencia de uso: OPERACIÓN (siempre visible: Dashboard, Conversaciones, Comprobantes y la página del rubro), VENTAS Y MARKETING (CRM, Campañas, Embudo, Productos), AGENTE IA (Agente IA, Flujos, Recordatorios, Pruebas) y CONFIGURACIÓN (Empresa, Pagos, WhatsApp API, Chat Web, Integraciones, Mi plan) — estos dos últimos grupos aparecen COLAPSADOS por defecto: si el usuario 'no encuentra' una página, dile que despliegue el grupo tocando su título. Centro de ayuda está fijo al PIE del menú, y Activación desaparece del menú al completar el setup:",
  "- Activación (/activacion): checklist de puesta en marcha con % de avance y botón de capacitación 1:1 por WhatsApp. Desaparece del menú al llegar al 100%.",
